...
import { features, details } from "./constance";
  ...
  return (
    <section>
      <Element name="features">
        <div className="container  lg:px-28  max-lg:px-5">
          <div className="relative flex   md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            {features.map(({ id, icon, caption, title, text, button }) => (
              ...
              >
                ...
                <p className=" text-primary-600 font-semibold  mb-5 max-md:mb-6">
                  {caption}
                </p>
                <h2 className="max-w-400 mb-7 h3 text-p4 max-md:mb-6 max-md:h5">
                  {title}
                </h2>
                <p className="mb-11 body-1 max-md:mb-8 max-md:body-3 text-p4">
                  {text}
                </p>
                {!isLoading && isVerified ? (
                  <div className="flex gap-x-10 mx-[10px] mt-3">
                    <LinkNavigate to={`/${user.role.toLowerCase()}/dashboard`}>
                    <Button icon={button.icon}>ورود به داشبورد</Button>
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-16 px-4 pb-10">
